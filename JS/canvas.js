(function () {
  ...
  function drawFlowers() {
    ...
    positions = [];
    body.style.filter = chance(0.12)
      ? `grayscale(${getRandomInt(0, 80)}%)`
      : "";
    let mode = pick([
      "daisies",
      "multi",
    ...
    var colorz = {
      ...
      crosshatch: {
        ...
        vary: 80,
        mix: true,
      } /*only defines one varying pistil color*/,
      greenRed: {
        h: 102,
        s: 72,
        l: 27,
    ...
    var opt = {
      ...
      innerDetails: chance(),
      petalIterations: 4,
      minOpacity: getRandomFloat(0.5, 0.9),
      invertColors: chance(0.25),
    };
    ...
    let colors =
      getThemeColors(
        colorOpt
      ); /*pistil color at index 0, petal color at index 1*/
    ...
    if (opt.invertColors) {
      ...
      if (colorOpt && colorOpt.mix) {
        /*switching petals and pistil color*/
        petalC2 = colors[0].hsl();
        pistilC2 = colors[1].hsl();
      } else {
    ...
    //draw the flowers
    let numFlower =
      mode == "multi" ? getRandomInt(50, 1000) : getRandomInt(40, 250);
  ...
  function drawFlower(opt) {
    ...
    for (let i = 0; i < f.petals; i++) {
      ...
      for (let j = 0; j < opt.petalIterations; j++) {
        ...
        ctx.translate(f.x, f.y);
        if (j % 2 == 0) {
          /*symmetric rotations for more symmetric petals, rando for some irregularity*/
    ...
    for (let j = 0; j < 3; j++) {
      ctx.arc(
        f.x + getRandomFloat(-2, 2),
        f.y + getRandomFloat(-2, 2),
        f.r / f.centerDivisor,
        0,
        360
      );
    ...
    ctx.fillStyle =
      opt.mode != "crosshatch" && opt.mode != "daisies"
        ? chance()
          ? pistilC2
          : pistilC
        : pistilC;
  ...
  function Flower(opt) {
    ...
    this.petals =
      this.r <= 40 ? getRandomInt(3, 9) : getRandomInt(MINPETAL, MAXPETAL);
    ...
    this.petalSpread1 = getRandomFloat(0.3, this.petals <= 7 ? 0.7 : 0.5);
    this.petalLength1 = getRandomFloat(0.9, 2);
    this.petalSpread2 = getRandomFloat(
      this.petalSpread1 - 0.2,
      this.petalSpread1 + 0.2
    );
    this.petalLength2 = getRandomFloat(0.9, 2);
